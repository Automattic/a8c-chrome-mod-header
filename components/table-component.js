...
class TableComponent extends HTMLElement {
    ...
    reRender(event) {
        ...
        if (event.target.matches('.a8c-add__button, .a8c-add__button *') || event.target.matches('.deleteHeader, .deleteHeader *') ) {
            this.render();
        }
    }
    ...
    // delete row
    deleteRow(event) {
        if (event.target.classList.contains('deleteHeader')) {
            ...
            chrome.storage.sync.set({ customHeaders: JSON.stringify(this.headersData) }, () => {
                chrome.runtime.sendMessage({ type: 'updateRules', customHeaders });
            });
        }
    }
    ...
    Row(key = 0, header = { enabled: false, name: '', value: '' }, isAddRow = false) {
        ...
        return `
        <tr id="tr-${key}">
            <td><input type="checkbox" class="enableHeader" ${header?.enabled ? 'checked' : ''} /></td>
